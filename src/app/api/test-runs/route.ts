...
export async function GET(request: Request) {
  return withApiHandler(
    ...
    async ({ correlationId }) => {
      ...
      const runs = await listTestRunsByPrompt(parsedQuery.data.promptId);
...
export async function POST(request: Request) {
  return withApiHandler(
    ...
    async ({ correlationId }) => {
      ...
      const promptRow = await findPromptById(payload.promptId);
      ...
      const versionRow = await findPromptVersionById(targetVersionId);
      ...
      const runningRun = await createRunningTestRun({
      ...
      try {
        ...
        const completedRun = await markTestRunSucceeded(runningRun.id, response.output_text, response.usage);
        ...
        await markTestRunFailed(runningRun.id, errorMessage);
