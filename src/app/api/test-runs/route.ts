...
export async function GET(request: Request) {
  ...
  if (!parsedQuery.success) {
    return apiError("INVALID_INPUT", "promptId query param is required", 400);
  }
  ...
  try {
    ...
    return apiError("INTERNAL_ERROR", "Failed to list test runs", 500);
  }
}
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return apiError("INVALID_INPUT", "Invalid test run payload", 400);
  }
  ...
  try {
    ...
    if (!targetVersionId) {
      return apiError("INVALID_INPUT", "Prompt has no version to run", 400);
    }
    ...
    if (!versionRow || versionRow.promptId !== payload.promptId) {
      return apiError("NOT_FOUND", "Prompt version not found", 404);
    }
    ...
    if (!runningRun) {
      return apiError("INTERNAL_ERROR", "Failed to create test run", 500);
    }
    ...
    try {
      ...
      if (!completedRun) {
        return apiError("INTERNAL_ERROR", "Failed to finalize test run", 500);
      }

      return apiOk(completedRun, { status: 201 });
    } catch (error) {
      ...
      return apiError("OPENAI_ERROR", "Model invocation failed", 502);
    }
  } catch (error) {
    console.error("Failed to run prompt test", error);
    return apiError("INTERNAL_ERROR", "Failed to run prompt test", 500);
  }
}
